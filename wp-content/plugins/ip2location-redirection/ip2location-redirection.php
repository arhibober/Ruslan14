...
add_action('wp_ajax_ip2location_redirection_update_database', [$ip2location_redirection, 'download_database']);
...
class IP2LocationRedirection
{
	...
	public function plugin_enqueues($hook)
	{
		...
		if ($hook == 'toplevel_page_ip2location-redirection') {
			...
			wp_enqueue_script('ip2location_redirection_admin_script', plugins_url('/assets/js/feedback.js', __FILE__), ['jquery'], null, true);
	...
	public function download_database()
	{
		...
		try {
			...
			$isIP2LocationBIN = preg_match('/IP2LOCATION/', $bin_database);
			$files = scandir(IP2LOCATION_DIR);

			// Remove existing BIN files
			foreach ($files as $file) {
				if (strtoupper(substr($file, -4)) == '.BIN') {
					if ($isIP2LocationBIN && preg_match('/IP2LOCATION/', $file)) {
						$wp_filesystem->delete(IP2LOCATION_DIR . $file);
					}

					if (!$isIP2LocationBIN && preg_match('/IP2PROXY/', $file)) {
						$wp_filesystem->delete(IP2LOCATION_DIR . $file);
					}
				}
			}
			...
			switch ($tab) {
						...
						if (!filter_var($ip_address, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
							...
							if (empty($result['country_code'])) {
								...
								<div id="message" class="updated">
									<p>IP address <code>' . $ip_address . '</code> belongs to <strong>' . $result['country_name'] . ' (' . $result['country_code'] . ')' . (($result['region_name']) ? (', ' . $result['region_name']) : '') . '</strong>.</p>
								</div>';
							}
						}
					}
				...
				case 'settings':
					...
					<div class="wrap">
							...
							<table class="form-table">
								...
								<tr>
									<th scope="row" colspan="2">
										<label for="enable_region_redirection"><input type="checkbox" name="enable_region_redirection" id="enable_region_redirection" value="true"' . (($enable_region_redirection) ? ' checked' : '') . '> Enable Region Redirection</label>
									</th>
								</tr>
							</table>
						...
						<div id="bin_database">
							...
							' . var_dump($this->is_region_supported()) . '
							' . (($lookup_mode == 'bin' && get_option('ip2location_redirection_enable_region_redirect') && $this->is_region_supported() == false) ? '
							<div style="color:#cc0000">
								<p>Region information is not available in the current database. Please download the DB3 or DB3 LITE database, otherwise the redirection will not be working properly.</p>
							</div>' : '') . '
								...
								<tr>
									...
									<td>
										<select name="database_name" id="database_name">
											<option value=""></option>
											' . ((get_option('ip2location_redirection_enable_region_redirect')) ? '
											<option value="DB3LITEBIN"> IP2Location LITE DB3</option>
											<option value="DB3BIN"> IP2Location DB3</option>
											<option value="DB3LITEBINIPV6">IP2Location LITE DB3 (IPv6)</option>
											<option value="DB3BINIPV6">IP2Location DB3 (IPv6)</option>
											' : '
				...
				default:
					...
					<div class="wrap">
						...
						<input type="hidden" id="enable_region_redirection" value="' . (get_option('ip2location_redirection_enable_region_redirect') ? 'true' : 'false') . '">
	...
	public function redirect()
	{
		...
		if (($data = json_decode(get_option('ip2location_redirection_rules'))) !== null) {
			$result = $this->get_location($this->get_ip(), false);
			...
			foreach ($data as $values) {
				...
				if ($this->is_country_match($result, $country_codes)) {
					...
					$this->write_debug_log('Result       : Country not matched');
				}
	...
	public function admin_footer_text($footer_text)
	{
		...
		if (($current_screen && strpos($current_screen->id, $plugin_name) !== false)) {
			$footer_text .= sprintf(
				...
				'<strong>' . __('IP2Location Redirection', $plugin_name) . '</strong>',
		...
		if ($current_screen->id == 'plugins') {
			return $footer_text . '
			<div id="ip2location-redirection-feedback-modal" class="hidden" style="max-width:800px">
				...
				<p>
					<label>
						<input type="radio" name="ip2location-redirection-feedback" value="1"> I no longer need the plugin
					</label>
				</p>
				<p>
					<label>
						<input type="radio" name="ip2location-redirection-feedback" value="2"> I couldn\'t get the plugin to work
					</label>
				</p>
				<p>
					<label>
						<input type="radio" name="ip2location-redirection-feedback" value="3"> The plugin doesn\'t meet my requirements
					</label>
				</p>
				<p>
					<label>
						<input type="radio" name="ip2location-redirection-feedback" value="4"> Other concerns
						<br><br>
				...
				<p>
					<div style="float:left">
						<input type="button" id="ip2location-redirection-submit-feedback-button" class="button button-danger" value="Submit & Deactivate" />
					</div>
	...
	private function get_location($ip, $use_cache = true)
	{
		...
		if (isset($_SESSION[$ip . '_country_code']) && !empty($_SESSION[$ip . '_country_code']) && $use_cache) {
			...
			$this->write_debug_log('Country      : ' . $_SESSION[$ip . '_country_name'] . ' [' . $_SESSION[$ip . '_country_code'] . (($_SESSION[$ip . '_region_code']) ? (', ' . $_SESSION[$ip . '_region_code']) : '') . ']');

			return [
				'country_code' => $_SESSION[$ip . '_country_code'],
				'country_name' => $_SESSION[$ip . '_country_name'],
				'region_code'  => $_SESSION[$ip . '_region_code'],
				'region_name'  => $_SESSION[$ip . '_region_name'],
			];
		}
		...
		switch (get_option('ip2location_redirection_lookup_mode')) {
			...
			case 'ws':
				...
				$_SESSION[$ip . '_region_code'] = $this->get_region_code($json->country_code, $json->region_name);
				$_SESSION[$ip . '_region_name'] = $json->region_name;
			...
			case 'bin':
				...
				$_SESSION[$ip . '_region_code'] = '';
				$_SESSION[$ip . '_region_name'] = '';

				if (!preg_match('/unavailable/', $response['regionName'])) {
					$_SESSION[$ip . '_region_code'] = $this->get_region_code($response['countryCode'], $response['regionName']);
					$_SESSION[$ip . '_region_name'] = $response['regionName'];
				}
